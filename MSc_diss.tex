...
\begin{itemize}
\item $NO_3$ radical rapidly photolyzes during the daylight hours, but in nighttime its concentration can increase to measurable levels \citep{Atkinson2008}. This increase in concentration and a lesser availability of the OH radical (which sources are mostly photolytic in nature) makes $NO_3$ radical a major contributor to the night chemistry of organics in the troposphere \citep{Finlayson-Pitts2000}. However, exclusion of $NO_3$ radical reactions from our chemical mechanism should not have a huge impact on the results anyway since the mechanism considers mainly decomposition of alkanes with which $NO_3$ radical reacts to a generally minor extent \citep{Atkinson2008}???.
...
\item Peroxyacetyl nitrates ($RC(O)OONO_2$, PAN)
is important as a means of transporting NOx over large distances. It is quite stable at low temperatures but decomposes at higher temperatures to release NO2. Hence whether photolysis can compete with the thermal decomposition is of great interest.

The rate constant for PAN decomposition is strongly temperature dependent. As a result, this is important in the atmosphere because PAN acts as a reservoir for NO2. At low temperatures, much of the total NOx may be tied up in PAN; as the atmosphere warms, due to either duirnal temperature variations or the air mass being transported into warmer regions. the rate of PAN decomposition increases. This releases NO2, which can then form other secondary pollutants such as O3 and HNO3.

PAN's thermal decomposition releases both NO2 and an organic free radical (acetylperoxy radical), so that it can act as an NOx reservoir and ultimately as a source of OH in the dark.
The oxidation of organics produces a variety of free radicals, some of which can form peroxyacyl nitrates. For example, acetaldehyde is a classic precursor to the PAN through the following reaction sequence:
CH3CHO + OH = CH3CO + H2O; 
CH3CO + O2 + M = CH3C(O)OO + M;
CH3C(O)OO + NO2 + M = CH3C(O)OONO2 + M.
The key feature of the last reaction is that it is reversible.
Of all the possible fates for PANs in the atmosphere, thermal decomposition is usually the most important.
The strong dependence has important implications for the role of PAN. When it is formed at lower temperatures or is transported into colder regions, it is stabilized and acts as an NOx reservoir. When an air mass containing PAN is transported into warmer regions, however, the PAN decomposes, releasing both the NO2and the acetylperoxy free radical. If sufficient NO is present, the organic free radical reacts int the following manner:
CH3C(O)OO + NO = CH3C(O)O + NO2;
CH3C(O)O = CH3 + CO2
followed by the reaction of CH3 with O2, etc. to form HCHO and HO2, or HCHO, CH3OH and CH3OOH under low-NOx conditions. As a result, PAN should act as an accelerator for photochemical smog formation.
At lower temperatures such as those found in the Arctic, the majority  of NOy is often present in the form of PAN because of its thermal stability.
\end{itemize}
