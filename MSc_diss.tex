...
A one (0D) box model is developed to investigate the impact of alkyl nitrate chemistry on ozone production efficiency. It is written from scratch using FACSIMILE for Windows. This special high-level programming language is a powerful modelling tool designed to efficiently solve differential equations including those resulting from a set of chemical reactions of interest.

The model is intended to represent the steady state of the troposphere under mean summer daytime meteorology and clear skies. To achieve that the air temperature is fixed at $25^{\circ}C$ and relative humidity at 70\%. There is no transport and mixing in the model.

\subsection{Chemical mechanism}
The chemical mechanism of the model is based on recommendations from \citep{Atkinson2004} and the Master Chemical Mechanism (http://mcm.leeds.ac.uk/MCM/home.htt). On a whole it includes 90 species and 165 reactions. It is focused on representation of the chemistry related to $O_3$ production, namely:
\begin{itemize}
\item $O_3$ null cycle, HOx and NOx cycles;
\item oxidation of methane ($CH_4$) and other alkanes, namely ethane ($C_2H_6$), propane ($C_3H_8$), n-butane ($nC_4H_{10}$), i-butane ($iC_4H_{10}$), n-pentane ($nC_5H_{12}$) and i-pentane ($nC_5H_{12}$)
\item alkyl nitrate chemistry.
\end{itemize}

Photolysis rates are calculated externally using parameters from MCMv3.3.
...
\begin{tabular}{ccc}
\hline
Compound       & Initial concentration, ppb \\
\hline
$O_3$          & 40 \\
$CO$           & 100 \\
\hline
...
\caption{Model setup: NOx initial concentrations (in ppb)}
\centering
\begin{tabular}{ccc}
\hline
$NO$      & $NO_2$      & NOx  ($=NO+NO_2$) \\
